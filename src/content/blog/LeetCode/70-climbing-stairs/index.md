...
In the function the first step is for a if statement to check if the given input is equal to 1, if so then `return` `1`, there is only  one combination for that 1. Next, create and initialize two variables, `first` to `1` and `second` to `2` . 
